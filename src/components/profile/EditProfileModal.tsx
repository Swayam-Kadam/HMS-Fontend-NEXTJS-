...
const toDateInputValue = (dateOfBirth: string) => {
...
const EditProfileModal = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (form.name !== profile.name) formData.append('name', form.name);
    if (form.address !== profile.address) formData.append('address', form.address);
    if (form.gender !== profile.gender) formData.append('gender', form.gender);
    if (form.bloodGroup !== profile.bloodGroup) {
      formData.append('bloodGroup', form.bloodGroup);
    }
    if (toDateInputValue(form.dateOfBirth) !== toDateInputValue(profile.dateOfBirth)) {
      formData.append('DOB', toDateInputValue(form.dateOfBirth));
    }
    if (form.bloodDonation !== profile.bloodDonation) {
      formData.append('donation[blood]', String(form.bloodDonation));
    }
    if (form.heartDonation !== profile.heartDonation) {
  ...
  const currentImage = previewUrl || profile.userImage || '';
        ...
        <div className="pt-2 border-t border-gray-100">
          ...
          <div className="space-y-3 mt-2">
            <label className="flex items-center gap-3 p-3 rounded-xl border border-gray-100 bg-gray-50 cursor-pointer hover:bg-gray-100 transition">
              ...
              />
              <span className="text-sm font-medium text-gray-700">Registered blood donor</span>
            </label>
            <label className="flex items-center gap-3 p-3 rounded-xl border border-gray-100 bg-gray-50 cursor-pointer hover:bg-gray-100 transition">
              <input
                type="checkbox"
                ...
                className="w-4 h-4 text-blue-600 rounded border-gray-300 focus:ring-blue-500"
              />
              <span className="text-sm font-medium text-gray-700">Heart organ donor pledge</span>
            </label>
          </div>
        </div>
      </form>
